...
function DoctorsDropdown() {
  ...
  useEffect(() => {    
    fetch("https://regent-hms-group-11.onrender.com/docInfo")
    .then(res => res.json())
    .then(res => {
      let arr = []
      res.data.forEach(i => {
            ...
            onSubmit={({ value }) => {
              ...
              fetch("https://regent-hms-group-11.onrender.com/userInSession")
                ...
                .then(res => {
                  ...
                  fetch("https://regent-hms-group-11.onrender.com/checkIfApptExists?email=" + email_in_use + "&startTime=" + theTime + "&date=" + theDate + "&docEmail=" + theDoc)
                    .then(res => res.json())
                    .then(res => {
                      if ((res.data[0])) {
                        window.alert("Appointment Clash! Try another doctor or date/time");
                      } else {
                        fetch("https://regent-hms-group-11.onrender.com/genApptUID")
                          .then(res => res.json())
                          .then(res => {
                            var string_json = JSON.stringify(res);
                            ...
                            console.log(gen_uid);
                            fetch("https://regent-hms-group-11.onrender.com/schedule?time=" + theTime + "&endTime=" + endTime +
                              "&date=" + theDate + "&concerns=" + theConcerns + "&symptoms=" + theSymptoms + 
                              "&id=" + gen_uid + "&doc=" + theDoc).then((x)=>{
                              fetch("https://regent-hms-group-11.onrender.com/addToPatientSeeAppt?email=" + email_in_use + "&id=" + gen_uid +
